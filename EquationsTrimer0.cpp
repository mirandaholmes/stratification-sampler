...
void Stratification::computestats(Point& x, VectorXd& stats){
    ...
    stats(0) = x.neqns;  // energy (not that meaningful)
    stats(1) = Cluster::dist(x.p,0,2,dim);  // distance 1-3
    stats(2) = x.dim;    // = 1 if bonded
    stats(3) = Cluster::costh(x.p,0,1,2,dim);  // internal angle
}
